class UserDevice < ApplicationRecord
    ...
    def self.save_device_information(user_id, device_id, platform, device_name,token)
     ...
     def self.send_estimate_time_out_notification
        ...
        @time_entries = TimeEntry.where.not(estimated_time_out: nil).where("DATE(date_of_activity) = ? and time_out  IS NULL", Time.now.in_time_zone.strftime("%Y-%m-%d")).joins(:user=> :user_devices).select("time_entries.id as entry_id, week_id, project_id,task_id,estimated_time_out,time_entries.user_id , user_devices.user_token as user_token")
        push_messages = []
        @time_entries.each do |time_entry_details|   
            if current_time >= time_entry_details.estimated_time_out.in_time_zone(time_zone).time.strftime("%k:%M")
                token = time_entry_details.user_token
                push_messages.push({
                    to: token,
